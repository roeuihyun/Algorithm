 ...
 */
public class _2455IntelligentTrain {
    public static void main(String[] args){
        int passenger[][] = new int[4][2];
        int totalPassenger = 0;
        int maxPassengerPlatform = Integer.MIN_VALUE;
        Scanner sc = new Scanner(System.in);
        for(int i = 0; i < 4; i ++){
            for(int j = 0; j < 2; j ++){
                passenger[i][j] = sc.nextInt();
                if(j == 0){
                    totalPassenger -= passenger[i][j];
                }else if(j == 1){
                    totalPassenger += passenger[i][j];
                    if(maxPassengerPlatform > totalPassenger){
                        maxPassengerPlatform  = totalPassenger;
                    }
                }
            }
        }

        System.out.println(maxPassengerPlatform);
