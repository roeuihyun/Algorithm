...
public class StringExercise {
    ...
    String getMiddle(String word){
    	return ((word.length() & 1) == 0) ? new StringBuffer().append(word.charAt((word.length()/2)-1)).append(word.charAt((word.length()/2))).toString() 
    									  : new StringBuffer().append(word.charAt((word.length()/2))).toString();  
    }
